...
for tool in node python3 docker; do
...
### Tool Verification Script (Node.js)
```javascript
#!/usr/bin/env node
...
const requiredTools = [
  { name: 'node', versionCmd: 'node --version', minVersion: '18.0.0' },
  { name: 'npm', versionCmd: 'npm --version', minVersion: '9.0.0' },
  { name: 'git', versionCmd: 'git --version', minVersion: '2.30.0' },
];

function checkTool(tool) {
  try {
    const version = execSync(tool.versionCmd, { encoding: 'utf-8' }).trim();
...
const allPresent = requiredTools.every(checkTool);
